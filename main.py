...
def start_page():
    if(request.method == "GET"):
        return render_template("index.html", isNotLogin = False)
    else:
        data = request.form
        if(auth(data["username"], data["password"])):
            ...
            return render_template("lk.html", name = session["username"])
        else:
            return render_template("index.html", isNotLogin = True)
